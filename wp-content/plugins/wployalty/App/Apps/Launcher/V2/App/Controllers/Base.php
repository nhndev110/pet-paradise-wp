...
class Base {
	...
	function isLauncherSecurityValid( $nonce_name = '' ) {
		...
		if ( ! Woocommerce::hasAdminPrivilege() || ! Woocommerce::verify_nonce( $wll_nonce, $nonce_name ) ) {
			return false;
		}
	...
	public function getCampaigns() {
		if ( ! empty( self::$get_campaign_list ) ) {
			return self::$get_campaign_list;
		}
		$customer_page = new \Wlr\App\Controllers\Site\CustomerPage();
		$user_email    = self::$woocommerce->get_login_user_email();
		...
		switch ( $action_type ) {
			...
			case 'referral':
				...
				if ( $referral_url_check ) {
					$active_campaigns->referral_url = $base_helper->getReferralUrl( $user->refer_code );
				}
				break;
			case 'facebook_share':
			case 'twitter_share':
	...
	public function getSocialIconList( $user_email, $url ) {
		if ( ! empty( self::$social_share_list ) ) {
	...
	function getRewardOpportunities() {
		if ( ! empty( self::$reward_opportunities ) ) {
			return self::$reward_opportunities;
		}
		$user_email    = self::$woocommerce->get_login_user_email();
		...
		return self::$reward_opportunities = apply_filters( 'wll_before_launcher_reward_opportunities', array(
	...
	function getString( $key, $default, $saved ) {
		...
		if ( strpos( $key, "." ) !== false ) {
			...
			if ( empty( $value ) ) {
				$value = $base_helper->getOptValue( $default, $identifiers );
			}
		}
