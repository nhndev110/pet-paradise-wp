...
class Settings extends Base {
	...
	public function getShortCodeList( $is_admin_page = true ) {
		if ( ! empty( self::$short_code_list ) ) {
			return apply_filters( 'wlr_launcher_short_code_list', self::$short_code_list );
		}
		$earn_campaign_helper = EarnCampaign::getInstance();
		...
		$short_code_list      = array(
			...
			'{wlr_user_name}'                          => '',
			'{wlr_user_points}'                        => 0,
			'{wlr_point_label}'                        => $earn_campaign_helper->getPointLabel( 3 ),
	...
	protected function getActiveReferralCampaigns() {
		if ( ! empty( self::$active_referral_campaign ) ) {
			return self::$active_referral_campaign;
		}
