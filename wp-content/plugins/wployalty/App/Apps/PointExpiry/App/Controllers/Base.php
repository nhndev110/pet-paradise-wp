...
class Base {
	...
	function addMenu() {
		if ( Woocommerce::hasAdminPrivilege() ) {
			add_menu_page( __( 'WPLoyalty: Point Expire', 'wp-loyalty-rules' ), __( 'WPLoyalty: Point Expire', 'wp-loyalty-rules' ), 'manage_woocommerce', WLPE_PLUGIN_SLUG, array(
				$this,
				'managePages'
		...
		if ( self::$input->get( 'page' ) == WLPE_PLUGIN_SLUG ) {
			...
			switch ( $view ) {
				...
				default:
					...
					$item_where         = $where . $order_by . $wpdb->prepare( ' LIMIT %d OFFSET %d', array(
							$per_page,
							$offset
						) );
					$point_expire_model = new ExpirePoints();
					...
					$params                          = array(
						...
						'perPage'     => $per_page,
						'baseURL'     => admin_url( 'admin.php?' . http_build_query( array(
								'page'       => WLPE_PLUGIN_SLUG,
								'view'       => 'expire_points',
					...
					$page_details                    = array(
						...
						'point_sort'        => $point_sort,
						'filter_status'     => array(
							'all'     => __( 'All', 'wp-loyalty-rules' ),
							'open'    => __( 'Open', 'wp-loyalty-rules' ),
	...
	function updateExpiryDate() {
		...
		if ( $row_id > 0 && ! empty( $action_type ) && in_array( $action_type, array( 'point', 'email' ) ) ) {
			...
			if ( $action_type === 'point' ) {
				...
				$update_date       = array(
					'status'      => 'active',
					'expire_date' => self::$woocommerce->beforeSaveDate( $expiry_point_date ),
				...
				$update_date       = array(
					'status'            => 'active',
					'expire_email_date' => self::$woocommerce->beforeSaveDate( $expiry_email_date ),
					'modified_at'       => $current_date
				);
				$message['data']   = array(
