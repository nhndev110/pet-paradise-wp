...
class Validation {
	...
	static function validateDashboard( $post, $task = '' ) {
		...
		if ( in_array( $task, array( 'getActivityLoyalData', 'getChartData', 'getCustomerRecentActivity' ) ) ) {
			Validator::addRule( 'alphaNumWithUnderscore', array(
				__CLASS__,
				'validateAlphaNumWithUnderscore'
			), __( 'accepts only letters,numbers and underscore', 'wp-loyalty-rules' ) );
			Validator::addRule( 'dateORNull', array(
				__CLASS__,
				'validateDateORNull'
	...
	static function validateCommonFields( $post ) {
		...
		$settings_validator->stopOnFirstFail( false );
		Validator::addRule( 'sanitizeText', array(
			__CLASS__,
		...
		Validator::addRule( 'alphaNumWithUnderscore', array(
			__CLASS__,
			'validateAlphaNumWithUnderscore'
		), __( 'validation has failed', 'wp-loyalty-rules' ) );
	...
	static function validateSettingsTab( $post ) {
		...
		$labels_array_fields = array(
			...
			'theme_color',
			'heading_color',
			'wlr_is_cart_earn_message_enable',
			'wlr_is_cart_redeem_message_enable',
			'wlr_is_checkout_redeem_message_enable',
			'wlr_is_thank_you_message_enable',
			'earn_cart_text_color',
			'earn_cart_border_color',
			'earn_cart_background_color',
			'redeem_cart_text_color',
			'redeem_cart_border_color',
		...
		$settings_validator->stopOnFirstFail( false );
		Validator::addRule( 'cleanHtml', array(
			__CLASS__,
			'validateCleanHtml'
		), __( 'Invalid characters', 'wp-loyalty-rules' ) );
		Validator::addRule( 'basicHtmlTags', array(
			__CLASS__,
			'validateBasicHtmlTags'
		), __( 'Invalid characters', 'wp-loyalty-rules' ) );
		Validator::addRule( 'alphaNumWithSpace', array(
			__CLASS__,
			'validateAlphaNumWithSpace'
		), __( 'accepts only letters,numbers and space', 'wp-loyalty-rules' ) );
		Validator::addRule( 'radioButtonAndSelectBox', array(
			__CLASS__,
			'validateRadioButtonAndSelectBox'
		), __( 'must contain any of the following values: yes,no,1,0', 'wp-loyalty-rules' ) );
		Validator::addRule( 'alphaNumWithUnderscore', array(
			__CLASS__,
			'validateAlphaNumWithUnderscore'
		), __( 'validation has failed', 'wp-loyalty-rules' ) );
		...
		Validator::addRule( 'limitLength', array(
			__CLASS__,
			'validateLimitLength'
		), __( 'Character length must be less than 20', 'wp-loyalty-rules' ) );
		Validator::addRule( 'hexColor', array(
			__CLASS__,
			'validateColor'
		), __( 'invalid color format use hex color', 'wp-loyalty-rules' ) );
		Validator::addRule( 'alphaUnderscoreHyphen', array(
			__CLASS__,
			'validateAlphaHyphenUnderscore'
		), __( 'Invalid characters', 'wp-loyalty-rules' ) );
		...
		$settings_validator->rule( 'hexColor', array(
			'redeem_button_color',
			'redeem_button_text_color',
			'apply_coupon_border_color',
			'apply_coupon_button_text_color',
		...
		$settings_validator->rule( 'alphaUnderscoreHyphen', array(
			'reward_code_prefix',
			'wlr_referral_prefix'
		) )->message( __( '{field} has invalid characters', 'wp-loyalty-rules' ) );
		if ( $settings_validator->validate() ) {
	...
	static function validateReward( $post ) {
		...
		Validator::addRule( 'inputAlpha', array(
			__CLASS__,
			'validateInputAlpha'
		), __( 'accepts only letters,numbers and underscore', 'wp-loyalty-rules' ) );
		Validator::addRule( 'alphaNumWithUnderscore', array(
		...
		Validator::addRule( 'number', array(
			__CLASS__,
			'validateNumber'
		), __( 'accepts only numbers', 'wp-loyalty-rules' ) );
		Validator::addRule( 'numberGeZero', array(
			__CLASS__,
		...
		Validator::addRule( 'nameLengthLimit', array(
			__CLASS__,
			'validateNameLimitLength'
		), __( ' is too length', 'wp-loyalty-rules' ) );
		Validator::addRule( 'isProductExist', array(
			__CLASS__,
			'checkIsProductExist'
		), __( 'product is not available', 'wp-loyalty-rules' ) );

		$rule_validator->rule( 'in', [ 'coupon_type' ], [
			'percent',
			'fixed_cart'
		] )->message( __( '{field} is invalid', 'wp-loyalty-rules' ) );
		...
		if ( isset( $post['discount_type'] ) && ! in_array( $post['discount_type'], array(
				'free_product',
				'free_shipping'
			) ) ) {
		...
		if ( isset( $post['conditions'] ) && is_array( $post['conditions'] ) && ! empty( $post['conditions'] ) ) {
			$condition_label_fields = array(
				'conditions.user_point.value',
				'conditions.user_point.operator',
				'conditions.user_level.value',
				'conditions.language.operator',
				'conditions.language.value',
				'conditions.currency.operator',
				'conditions.currency.value',
				'conditions.product_onsale.operator',
				'conditions.product_tags.operator',
				'conditions.product_tags.value',
				'conditions.product_tags.condition',
				'conditions.product_tags.qty',
				'conditions.cart_line_items_count.value',
				'conditions.cart_line_items_count.operator',
				'conditions.cart_line_items_count.sub_condition_type',
				'conditions.cart_weights.value',
				'conditions.cart_weights.operator',
				'conditions.cart_weights.sub_condition_type',
				'conditions.cart_subtotal.value',
				'conditions.cart_subtotal.operator',
				'conditions.products.value',
				'conditions.products.operator',
				'conditions.products.condition',
				'conditions.products.qty',
				'conditions.product_attributes.value',
				'conditions.product_attributes.condition',
				'conditions.product_attributes.qty',
				'conditions.product_attributes.operator',
				'conditions.product_category.value',
				'conditions.product_category.operator',
				'conditions.product_category.condition',
				'conditions.product_category.qty',
				'conditions.product_sku.value',
				'conditions.product_sku.operator',
				'conditions.product_sku.qty',
				'conditions.product_sku.condition',
				'conditions.payment_method.value',
				'conditions.payment_method.operator',
				'conditions.purchase_history.operator',
				'conditions.purchase_history.value',
				'conditions.purchase_history.order_status',
				'conditions.life_time_sale_value.operator',
				'conditions.life_time_sale_value.value',
				'conditions.life_time_sale_value.order_status',
	...
	static function validateRuleTab( $post ) {
		...
		Validator::addRule( 'checkOrderSpend', array(
			__CLASS__,
			'validateOrderAndSpend'
		), __( ' is required', 'wp-loyalty-rules' ) );
		Validator::addRule( 'nameLengthLimit', array(
			__CLASS__,
			'validateNameLimitLength'
		), __( 'is too length', 'wp-loyalty-rules' ) );
		...
		if ( isset( $post['conditions'] ) && is_array( $post['conditions'] ) && ! empty( $post['conditions'] ) ) {
			...
			$condition_label_fields = array(
				...
				'conditions.user_point.operator',
				'conditions.user_level.value',
				'conditions.usage_limits.value',
				'conditions.product_onsale.operator',
				'conditions.user_role.value',
				'conditions.user_role.operator',
				...
				'conditions.payment_method.value',
				'conditions.payment_method.operator',
				'conditions.order_status.value',
				'conditions.order_status.operator',
				'conditions.cart_line_items_count.value',
				'conditions.cart_line_items_count.operator',
				'conditions.cart_line_items_count.sub_condition_type',
				'conditions.cart_weights.value',
				...
				'conditions.product_tags.condition',
				'conditions.product_tags.qty',
				'conditions.usage_limit.operator',
				'conditions.usage_limit.value',
				'conditions.products.value',
				'conditions.products.operator',
				'conditions.products.condition',
				'conditions.products.qty',
				...
				'conditions.product_sku.qty',
				'conditions.product_sku.condition',
				'conditions.purchase_history_qty.order_status',
				'conditions.purchase_history_qty.product_action_type',
				'conditions.purchase_history_qty.value',
				'conditions.purchase_history_qty.purchase_before',
				'conditions.purchase_history_qty.operator',
				'conditions.purchase_history_qty.condition',
				'conditions.purchase_history_qty.qty',
				'conditions.purchase_first_order.value',
				'conditions.purchase_last_order.operator',
				'conditions.purchase_last_order.value',
				'conditions.purchase_last_order.status',
				'conditions.purchase_last_order_amount.operator',
				'conditions.purchase_last_order_amount.value',
				'conditions.purchase_last_order_amount.status',
				'conditions.purchase_previous_orders.operator',
				'conditions.purchase_previous_orders.value',
				'conditions.purchase_previous_orders.status',
				'conditions.purchase_previous_orders.time',
				'conditions.purchase_previous_orders_for_specific_product.operator',
				'conditions.purchase_previous_orders_for_specific_product.value',
				'conditions.purchase_previous_orders_for_specific_product.status',
				'conditions.purchase_previous_orders_for_specific_product.time',
				'conditions.purchase_previous_orders_for_specific_product.products',
				'conditions.purchase_quantities_for_specific_product.operator',
				'conditions.purchase_quantities_for_specific_product.value',
				'conditions.purchase_quantities_for_specific_product.status',
				'conditions.purchase_quantities_for_specific_product.time',
				'conditions.purchase_quantities_for_specific_product.products',
				'conditions.purchase_spent.operator',
				'conditions.purchase_spent.value',
				'conditions.purchase_spent.status',
	...
	static function validateEditLevelFields( $post ) {
		...
		Validator::addRule( 'isEmpty', array( __CLASS__, 'validateIsEmpty' ), __( ' is empty', 'wp-loyalty-rules' ) );
		Validator::addRule( 'levelPoints', array(
			__CLASS__,
			'validateLevelPoints'
		...
		if ( isset( $post["from_points"] ) && empty( $post["from_points"] ) ) {
			...
			if ( empty( $post["from_points"] ) && empty( $post["to_points"] ) ) {
				$rule_validator->rule( 'isEmpty', array(
					'from_points',
					'to_points'
				) )->message( __( '{field} can not be 0', 'wp-loyalty-rules' ) );
			}
	...
	static function validateAddNewCustomer( $post ) {
		...
		Validator::addRule( 'validateEmail', array(
			__CLASS__,
			'validateEmail'
		), __( '{field} has invalid email address', 'wp-loyalty-rules' ) );
		$required_fields = array(
			'user_email',
			'points'
		);
	...
	static function validateDateORNull( $field, $value, array $params, array $fields ) {
		if ( empty( $value ) || in_array( $value, array(
				...
				0,
				'-'
			) ) ) { //is_null($value) || $value == 0 || $value === 'null' || $value == '-'
			return true;
		}
		$isDate = false;
